...
EventManager::EventManager(): listeners(), events(), eventTime(), serverTime(nullptr), writeEvents(), playerIndex(-1) {
}
...
float EventManager::getLocalTime() {
    return serverTime ? serverTime->getTime() : 0.0f; // will cause bad things if timeline is not set
}
