...
int main(int argc, char** argv)
{
  ...
  sf::RenderWindow window(sf::VideoMode(800, 600), "An orange game");
  std::vector<GameObject *> allObjects;
  /*
  {
    // set all platforms (now collected in constructor)
  ...
  sf::Vector2f spawn(650.0f, 380.0f);
  Player* players[8];
  {
    for (int i = 0; i < 8; i++) {
    ...
    players[clientID]->setSpawn(spawn);
  ...
  const auto loopPhysics = [&localTime, &serverTime, &players, &clientID, &allObjects]() { // make into its own v8 script
    ...
    for (GameObject* obj : allObjects) {
      ...
      obj->lock();
      obj->gameMove(serverTime); // in progress FIX
  ...
  const auto loopDraw = [&sp, &window, &players, &localTime, &serverTime, &localState, &clientID]() {
    window.setActive(true);
    // draw everything
    window.clear(sf::Color::Cyan);
    // Draw/update shapes and collide with new player position
    for (auto & [key, obj] : GameObject::game_objects) {
      obj->lock();
      obj->draw(window);
      obj->unlock();
    }
  ...
  { // anonymous scope for managing handle scope
    ...
    ScriptListener *sl = new ScriptListener(sm, "handle_event");
    ...
    while (window.isOpen())
    {
          ...
          if (profileCounter >= 1000) {
            std::cout << "hey that's 1000 frames, in " << localTime.getTime() - profileTime << " seconds" << std::endl;
